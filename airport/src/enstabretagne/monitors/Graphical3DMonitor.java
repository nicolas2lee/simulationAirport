...
public abstract class Graphical3DMonitor implements GLEventListener, MouseListener, 
MouseMotionListener, KeyListener  {
	...
	public Graphical3DMonitor(HashMap<String,HashMap<String,Object>> loggersNames,int fps) {
	...
	void initSimEngine(SimScenario scenario){
		...
		engine.Init(currentScenario);
		currentScenario.Initialize(new SimScenarioInit(MoreRandom.globalSeed));
	...
	void terminateSimEngine(boolean restart) {
		...
		currentScenario.terminate(restart);
		currentScenario=null;
	}
	
	void terminateMonitor(boolean restart) {
	...
	public void display(GLAutoDrawable drawable) {
		...
		currentScenario.InterruptIn(updateDelay);
		engine.simulate();
	...
	public void createGui(SimScenario s) {
